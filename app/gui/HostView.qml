...
UiPage {
    ...
    heading: qsTr("This computer, available to you.")
    ...
    UiCard {
        ColumnLayout {
            ...
            RowLayout {
                Button { text: qsTr("Review permissions"); onClicked: navigateTo("qrc:/gui/SetupView.qml", "SetupView") }
                Button { text: qsTr("Manage devices"); onClicked: navigateTo("qrc:/gui/BindView.qml", "BindView") }
            }
        }
    }
    UiCard {
    ...
    UiCard {
        ColumnLayout {
            anchors.fill: parent; spacing: 12
            Button { id: advanced; text: checked ? qsTr("Hide compatibility & diagnostics") : qsTr("Compatibility & diagnostics"); checkable: true }
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Button { text: qsTr("Allow"); enabled: hostManager.canPair && /^\d{4}$/.test(pin.text) && peer.text.trim().length > 0; onClicked: { hostManager.pair(pin.text, peer.text); pin.clear() } }
                }
                ...
                Button { text: qsTr("Open host logs"); onClicked: hostManager.openLogs() }
            }
        }
    }
}
